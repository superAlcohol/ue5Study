...
DECLARE_MULTICAST_DELEGATE(FOnHpZeroDelete);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnHpChangedDelegate, float /*CurrentHp*/);
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnStatChangeDelete, const FABCharacterStat&/*BastStat*/, const FABCharacterStat& /*ModifierStat*/);
...
public:
	FOnHpZeroDelete OnHpZero;
	FOnHpChangedDelegate OnHpChanged;
	FOnStatChangeDelete OnStatChanged;
	...
	FORCEINLINE void HealHep(float InHealAmount) { CurrentHp = FMath::Clamp(CurrentHp + InHealAmount, 0, GetTotalStat().MaxHp); OnHpChanged.Broadcast(CurrentHp); }
...
protected:
	...
	// Transient 디스크에 저장하지 않음 // VisibleInstanceOnly 인스턴스마다 생성 
