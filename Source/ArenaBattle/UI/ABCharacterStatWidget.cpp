...
void UABCharacterStatWidget::UpdateStat(const FABCharacterStat& BasetStat, const FABCharacterStat& ModifierStat)
{
	for (TFieldIterator<FNumericProperty> PropIt(FABCharacterStat::StaticStruct()); PropIt; ++PropIt)
	{
		const FName PropKey(PropIt->GetName());
		...
		float BaseData = 0.0f;
		PropIt->GetValue_InContainer((const void*)&BasetStat, &BaseData);
